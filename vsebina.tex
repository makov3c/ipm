...
Če ima kolobar $K$ enoto 1, rečemo, da je element $x  \in K$ \textbf{obrnljiv}, če $\exists y \in K\ :\ xy = 1$.\\
...
\end{multline*}
$\text{Ker} f = \{ x \in G | f(x) = e \}$ je jedro homomorfizma.

\subsection*{Ideali}
Podkolobar $I \leq K$ je ideal, če velja $I\cdot K \subseteq I$ in $K\cdot I \subseteq I$. Oznaka: $I \lhd K$.

V nekomutativnih kolobarjih ločimo \textbf{leve} in \textbf{desne} ideale. Primer: matrike, kjer so izbrani stolpci ničelni, so \textbf{levi} ideali. Matrike z ničelnimi vrsticami so \textbf{desni} ideali.\\

$K$ in $\{0\}$ sta \textbf{neprava ideala}.

(komutativen) kolobar $K$ je obseg $\iff$ nima pravih idealov. 

Še več,\textbf{ pravi ideali} ne vsebujejo obrnljivih elementov.
...
\textbf{Element je razcepen}, če ga lahko zapišemo kot $a \cdot b$, kjer $a$ in $b$ nista obrnljiva.\\
...
Množica neničelnih kvadratov v $\mathbb{Z}_p$ grupo za množenje moči $\frac{p-1}{2}$\\
Polinom $x^2 +1$ je nerazcepen v $\mathbb{Z}_p [x] \iff$ p oblike $p = 4k + 3$\\
\subsubsection*{Kandidati za ničle polinoma}

Naj bo $p(x) = a_k x^k + \dots + a_0 ; \quad a_i \in \mathbb{Z}$. \\
Kdaj je lahko $\frac{m}{n}; \ GCD(m,n) = 1$ ničla?\\
$$n | a_k \quad \text{in} \quad m | a_0$$

\textbf{$p(x)$ nima linearnih faktorjev $\iff$ nima ničel}\\

Ničle polinoma $x^p - x$ v $\mathbb{Z}_p$ , če je $p$ praštevilo:
...
Naj bo $a(x) = a_n x^n + \dots + a_0 \in \mathbb{Z}[x]$ polinom.\\
Če $\exists p \in \mathbb{P} : p | a_0, \dots, a_{n-1} \wedge p \nmid a_n \wedge p^2 \nmid a_0$\\ potem je $a(x)$ \textbf{nerazcepen} nad $\mathbb{Q}$.
...
največji skupni delitelj polinomov $p$ in $q$ je polinom $d = \text{gcd}(p,q)$, ki zadošča:\\
\begin{itemize}
	\item $d|p$ in $d|q$
	\item vsak polinom, ki deli $p$ in $q$ deli tudi $d$
	\item vodilni koeficient $d$ je enak $1$.
\end{itemize}
...
Na desni strani uporabimo normo in dobimo: $N(a+bi) \cdot N(c+di) = 1 \cdot 1 = 1$\\
$a + bi$ je obrnljivo $\iff N(a + bi) = 1$\\
(leži na enotski krožinici)\\
\textbf{Obrnljiva števila} so $1, -1, i, -i$\\
